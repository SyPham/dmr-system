...
import { MixingGlueService } from 'src/app/_core/_service/mixing-glue.service.js';
...
const SUPERVISOR = 2;
@Component({
  selector: 'app-mixing-glue',
  templateUrl: './mixing-glue.component.html',
  styleUrls: ['./mixing-glue.component.css']
})
export class MixingGlueComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private mixingGlueService: MixingGlueService
  ) {
    this.mixingGlueService.connect();
  }
  ngOnDestroy(): void {
  ...
  onBlur(data: IIngredient) {
    ...
    if (this.mixingGlueService.hubConnection.state === HubConnectionState.Connected) {
      this.signal();
    } else {
  ...
  private offSignalr() {
    this.mixingGlueService.hubConnection.off('Welcom');
  }
  private onSignalr() {
    this.mixingGlueService.hubConnection.on('Welcom', () => {});
  }
  ...
  private startScalingHub() {
    this.mixingGlueService.hubConnection.start().then(() => {
      this.mixingGlueService.hubConnection.on('Scaling Hub UserConnected', (conId) => {
        console.log('Scaling Hub UserConnected', conId);
        this.signal();
      });
      this.mixingGlueService.hubConnection.on('Scaling Hub User Disconnected', (conId) => {
  ...
  private signal() {
    this.mixingGlueService.getValue().subscribe( data => {
      if (data !== null) {
        const scalingMachineID = data.scalingMachineID;
  ...
  private signal2() {
    if (this.mixingGlueService.hubConnection.state === HubConnectionState.Connected) {
      this.mixingGlueService.hubConnection.on(
        'Welcom',
